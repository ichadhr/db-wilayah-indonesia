...
def kode_wilayah(raw_record: dict) -> KodeWilayah | None:
    ...
    cleaned = {
        ...
        'kabupaten_kota': str(processed.get('kabupaten_kota', '')).strip().replace('<br>', ' '),
        'nama_kota': str(processed.get('nama_kota', '')).strip().replace('<br>', ' '),
        'singkatan_nama_kota': str(processed.get('singkatan_nama_kota', '')).strip().replace('<br>', ' ').upper(),
        'parent_subdivision': str(processed.get('parent_subdivision', '')).strip().replace('<br>', ' ').upper(),
    }
