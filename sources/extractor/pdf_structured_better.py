...
class PDFStructureExtractor:
    ...
    def extract_structure(self) -> AdministrativeStructure:
        ...
        """
        # Process pages with enhanced progress tracking
            ...
            for page_num, page in enumerate(self.reader.pages):
                try:
                    ...
                    pbar.set_postfix_str(
                        f" {rate:.2f} page/s, pages={page_num + 1}, provinsi={len(self.administrative_structure)}, kabupaten_kota={sum(len(p.details) for p in self.administrative_structure)}"
                    )
                    pbar.update(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        import json
        with open("structure_output.json", "w", encoding="utf-8") as f:
