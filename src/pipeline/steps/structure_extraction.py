...
def execute_structure_extraction(
    ...
    force_regeneration: Optional[str] = None,
    logger=None,
) -> Optional[Any]:
    """
    ...
    if not os.path.exists(pdf_path):
        raise BatchProcessingError(
            f"PDF file not found: {pdf_path}", file_path=pdf_path
        )
    ...
    if not skip_cache:
        cached_structure = load_cached_structure_with_validation(
            structure_path, pdf_path
        )
        if cached_structure is not None:
            ...
            logger.info(
                "Cached structure exists but is invalid (PDF changed), regenerating"
            )
    ...
    with open(structure_path, "w", encoding="utf-8") as f:
        json.dump(
            structure.model_dump(exclude_none=False), f, ensure_ascii=False, indent=2
        )
