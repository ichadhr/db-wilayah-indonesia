...
def _extract_single_province_kabupaten_kota_detail(
    ...
    try:
        ...
        log_messages.append(
            f"INFO: Extracted {len(kabupaten_kota_detail)} records for {detail_name}"
        )
        ...
        log_error(
            BatchProcessingError(
                error_msg, province_name=province_name, detail_name=detail_name
            ),
            "kabupaten_kota_detail_extraction",
        )
...
def execute_kabupaten_kota_detail_batch(config, logger=None) -> Optional[Any]:
    ...
    if len(detail_df) == 0:
        raise BatchProcessingError(
            "No kabupaten/kota detail found in structure", file_path=structure_path
        )
    ...
    temp_parquet_path = get_parquet_output_path(
        "temporary_kabupaten_kota_detail.parquet", ensure_dir=True
    )
    ...
    for batch_start in range(0, total_details, config.batch_size):
        ...
        logger.info(
            f"Processing batch {batch_start // config.batch_size + 1}: details {batch_start + 1}-{batch_end} ({total_details})"
        )
        ...
        if config.max_workers > 1:
            results = processor.process_batch(
                pdf_path,
                batch_df,
                _extract_single_province_kabupaten_kota_detail,
                item_name="detail",
                log_filename=None,
            )
        else:
        ...
        successful_batch = [
            r for r in results if r["success"] and r.get("dataframe") is not None
        ]
        if successful_batch:
    ...
    if os.path.exists(temp_parquet_path):
        ...
        for province_name in provinces:
            ...
            parquet_path = get_parquet_output_path(
                os.path.join(folder_name_base, f"{filename}.parquet"), ensure_dir=True
            )
            province_data.write_parquet(parquet_path)
            files_generated.append(parquet_path)
            logger.info(
                f"Saved {province_name}: {parquet_path} ({len(province_data)} records)"
            )

            # Save debug JSON in province folder
            province_results = [
                r for r in successful if r["province_name"] == province_name
            ]
            debug_data = {
                "province_name": province_name,
                ...
                "total_extraction_time_seconds": sum(
                    r.get("time", 0.0) for r in province_results
                ),
            }
            debug_path = get_json_output_path(
                os.path.join("debug", folder_name_base, f"{filename}.json"),
                ensure_dir=True,
            )
    ...
    return type(
        ...
        (),
        {
            ...
            "metadata": {
                ...
                "provinces_processed": len(files_generated)
                // 2,  # Divide by 2 (parquet + json)
            },
