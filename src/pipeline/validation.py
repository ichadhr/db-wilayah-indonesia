    ...
    if "kode" in df.columns:
        ...
        for row in df.iter_rows(named=True):
            ...
            if code and (len(code) != 2 or not code.isdigit()):
                invalid_codes.append(f"{row.get('provinsi', 'Unknown')}: {code}")
        if invalid_codes:
            errors.append(f"Invalid province codes: {invalid_codes[:5]}")  # Show first 5

    # Check for duplicate province names or codes
    if "provinsi" in df.columns:
        names = [row["provinsi"] for row in df.iter_rows(named=True) if row.get("provinsi")]
        if len(names) != len(set(names)):
            duplicates = [name for name in names if names.count(name) > 1]
            errors.append(f"Duplicate province names: {list(set(duplicates))}")

    if "kode" in df.columns:
        codes = [str(row["kode"]) for row in df.iter_rows(named=True) if row.get("kode")]
