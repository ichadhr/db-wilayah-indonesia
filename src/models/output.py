...
class PostalMatcherOutput(BaseModel):
    ...
    parquet_dir: Path = Field(
        ..., description="Base directory containing province parquet files"
    )
    log_dir: Path = Field(..., description="Output directory for logs and results")

    # File suffixes
    detail_suffix: str = Field(
        "_kabupaten_kota_detail.parquet", description="Suffix for detail parquet files"
    )
    pos_suffix: str = Field(
        "_kabupaten_kota_pos.parquet", description="Suffix for POS parquet files"
    )
