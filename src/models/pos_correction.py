...
class NormalizedDetailRecord(BaseModel):
    ...
    original_provinsi: str = Field(
        ..., description="Original province name from source"
    )
    ...
    kelurahan_desa_combined: str = Field(
        "", description="Combined kelurahan and desa for matching"
    )
...
class NormalizedPosRecord(BaseModel):
    ...
    # Normalized fields for matching
    kecamatan_normalized: str = Field("", description="Normalized kecamatan name")
    desa_kelurahan_normalized: str = Field(
        "", description="Normalized desa/kelurahan name"
    )
...
class MatchedRecord(BaseModel):
    ...
    provinsi_similarity: Optional[float] = Field(
        None, description="Province similarity score"
    )
    kabupaten_similarity: Optional[float] = Field(
        None, description="Kabupaten similarity score"
    )
    kecamatan_similarity: Optional[float] = Field(
        None, description="Kecamatan similarity score"
    )
    kelurahan_similarity: Optional[float] = Field(
        None, description="Kelurahan similarity score"
    )

    # POS source data
    pos_original_provinsi: Optional[str] = Field(
        None, description="POS original province"
    )
    pos_kelurahan_desa: Optional[str] = Field(
        None, description="POS kelurahan/desa name"
    )
...
class UnmatchedDetailRecord(BaseModel):
    ...
    potential_kecamatan_matches: Optional[str] = Field(
        None, description="Potential kecamatan matches from similarity analysis"
    )
...
class MatchResult(BaseModel):
    ...
    exact_matches: list[MatchedRecord] = Field(
        default_factory=list, description="Records matched exactly"
    )
    fuzzy_matches: list[MatchedRecord] = Field(
        default_factory=list, description="Records matched using fuzzy logic"
    )
    unmatched_detail: list[UnmatchedDetailRecord] = Field(
        default_factory=list, description="Detail records without matches"
    )
    unmapped_pos: list[UnmappedPosRecord] = Field(
        default_factory=list, description="POS records without matches"
    )
    ...
    unmatched_detail_count: int = Field(
        0, description="Number of unmatched detail records"
    )
