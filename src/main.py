...
MAX_WORKERS = 7  # Concurrent threads (memory efficient with shared memory)
...
def main():
    ...
    log_filename = os.path.join(log_dir, datetime.now().strftime("kabupaten_kota-%Y%m%d-%H%M%S.log"))
    ...
    extract_table_kabupaten_kota_index_batch(
        pdf_path,
        structure_json_path,
        province_filter=None,  # Can be set to PROVINCE_FILTER if needed
        batch_size=BATCH_SIZE,
        max_workers=MAX_WORKERS,
        log_filename=log_filename,
    )

    # extract_table_kecamatan_index(pdf_path, structure_json_path)
...
def extract_table_kabupaten_kota_index(file_path: str, structure_path: str):
    ...
    try:
        ...
        json_debug_base = os.path.join(
            DEBUG_JSON_FOLDER, folder_name_base, filename_base
        )
...
def extract_table_kabupaten_kota_index_batch(file_path: str, structure_path: str, province_filter: Optional[List[str]] = None, batch_size: int = 38, max_workers: int = 7, log_filename: Optional[str] = None):
    """
