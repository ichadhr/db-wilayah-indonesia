...
class PDFStructureExtractor:
    ...
    CODE_PATTERN = re.compile(r"(C\.[a-zA-Z]+\.\d+\))\s*(.+)", re.IGNORECASE)
    ...
    def _process_single_page(self, page, page_num: int) -> None:
        ...
        if self.current_province:
            ...
            if kecamatan_index_start is not None and page_num >= kecamatan_index_start:
                # Check for C.*.\d+) patterns to update kecamatan_index end
                kecamatan_index_patterns = self.CODE_PATTERN.findall(text)
                if kecamatan_index_patterns:
                    # Get province_letter from first detail
    ...
    def print_validation_result(validation: dict[str, Any]) -> None:
        """
        Print the validation result in a standardized format.
