...
Previously, there were 37 unmatched entries from batch processing, generated by the [`execute_kecamatan_batch()`](src/pipeline/steps/kecamatan_batch.py:93) function. These represented `ibukota_kabupaten_kota` names that failed to match after normalization, plus unmapped BSNI cities. All have been resolved through the CorrectionLoader implementation.
...
These corrections are documented in [`src/utils/correction.csv`](src/utils/correction.csv) and address the unmatched entries above. **Summary: 11 BSNI fixes, 28 kecamatan_index fixes (39 total corrections)**.
...
- **Key Files**: [`src/extractor/pdf_table_extractor.py`](src/extractor/pdf_table_extractor.py), [`src/utils/correction.csv`](src/utils/correction.csv)
...
- **Key Files**: [`src/extractor/kode_wilayah_ocr.py`](src/extractor/kode_wilayah_ocr.py), [`src/utils/correction.csv`](src/utils/correction.csv)
